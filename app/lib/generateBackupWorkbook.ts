...
// Shared logic: builds the workbook and returns a Buffer.
// Used by both the manual "Download Backup" button and the
// automatic daily Google Drive backup — keep both in sync by
// editing this one function only.
export async function generateBackupWorkbookBuffer(): Promise<Buffer> {
  ...
  bookings.forEach((booking: any, index: number) => {
    ...
    paymentsSheet.addRow({
      ...
      balanceAmount: booking.balanceAmount || 0,
      paymentStatus:
        Number(booking.balanceAmount || 0) > 0 ? "Balance Pending" : "Paid",
    });
  });
  ...
  revenueSheet.addRows([
    { srNo: 1, description: "Total Revenue", amount: totalRevenue },
    { srNo: 2, description: "Advance Received", amount: advanceReceived },
    { srNo: 3, description: "Pending Balance", amount: pendingBalance },
  ]);
